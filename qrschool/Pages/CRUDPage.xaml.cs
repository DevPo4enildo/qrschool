    ...
    public partial class AddEquipmentViewModel : ObservableObject
    {
        ...
        private async Task OnSaveAsync()
        {
            try
            {
                ...
                {
                    await Application.Current!.MainPage!.DisplayAlert("Ошибка", "Выберите тип техники", "OK");
                    return;
                }

                if (string.IsNullOrWhiteSpace(Equipment.InventoryNumber))
                {
                    await Application.Current!.MainPage!.DisplayAlert("Ошибка", "Введите инвентарный номер", "OK");
                    return;
                }

                if (string.IsNullOrWhiteSpace(Equipment.Office))
                {
                    await Application.Current!.MainPage!.DisplayAlert("Ошибка", "Выберите кабинет", "OK");
                    return;
                }

                if (string.IsNullOrWhiteSpace(Equipment.Status))
                {
                    await Application.Current!.MainPage!.DisplayAlert("Ошибка", "Выберите статус", "OK");
                    return;
                }
                ...
                if (result)
                {
                    await Application.Current!.MainPage!.DisplayAlert("Успех", "Техника успешно сохранена!", "OK");
                    Equipment = new Equipment();
                    await Application.Current!.MainPage!.Navigation.PopAsync();
                }
                else
                {
                    await Application.Current!.MainPage!.DisplayAlert("Ошибка", "Не удалось сохранить технику", "OK");
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task OnCancelAsync()
        {
            bool confirm = await Application.Current!.MainPage!.DisplayAlert(
                "Подтверждение",
                "Отменить добавление техники? Несохранённые данные будут потеряны.",
                "Да",
                "Нет");
        ...
        private async Task OnAddNewOfficeAsync()
        {
            ...
            if (!string.IsNullOrWhiteSpace(newOffice))
            {
                ...
                {
                    await Application.Current!.MainPage!.DisplayAlert("Внимание", "Такой кабинет уже существует", "OK");
                }
            }
        }
